...
/**
 * This class handles messages sent from an Activity in a pool of threads
 * managed by the Java ExecutorService.
 */
class RequestHandler extends Handler {
	...
	/**
	 * Store a WeakReference to the Service to enable garbage collection.
	 */
	WeakReference<DownloadImagesBoundService> mService;

	/**
	 * Reference to the ExecutorService that manages a pool of threads.
	 */
	...
	/**
	 * Hook method called back when a request message arrives from an Activity.
	 * The Message it receives contains the Messenger used to reply to the
	 * Activity and the URL of the image to download. This image is stored in a
	 * local file on the local device and image file's URI is sent back to the
	...
	public void handleMessage(Message message) {
		...
		final Runnable downloadImageAndReply = new Runnable() {
			...
			public void run() {
				...
				Uri downloadedImageUri = Utils.downloadImage(context, imageUrl,
						dirPathname);
				...
				if (downloadedImageUri != null) {
					sendPath(replyToMessenger, downloadedImageUri, imageUrl,
							Activity.RESULT_OK);
				} else {
					sendPath(replyToMessenger, downloadedImageUri, imageUrl,
	...
	/**
	 * Send the @a pathToImageFile, @a url, and @a requestCode back to the
	 * Activity via the @a messenger.
	 */
	public void sendPath(Messenger messenger, Uri pathToImageFile, Uri url,
			int requestCode) {
		// Call the makeReplyMessage() factory method to create
		// Message.
		// TODO DONE-- you fill in here.
		ReplyMessage replyMsg = ReplyMessage.makeReplyMessage(pathToImageFile,
				url, requestCode);
		try {
			Log.d(TAG, "sending " + pathToImageFile
